...
const createStaff = async (req,res) => {
  const{name,email, username, pw, user_id} = req.body
  ...
  try{
    //const user_id = req.admin._id
      ...
      const staffMember = await Staff.creatingStaff(name, email, username, pw,user_id)
      ...
      res.status(200).json({staffMember,userid,eid})
...
const updateStaff = async (req, res) => {
  const { id } = req.params;
  const{name, NIC, Eid, faculty, contact, post, email, username} = req.body
