...
const addEnroll = async (req, res) => {
  try {
    const { userid, course, state } = req.body;

    if (!userid || !course) {
      return res
        .status(400)
        .json({ message: "Both userid and course are required." });
    }
    ...
    res
      .status(500)
      .json({ message: "Server Error - Unable to add Enrollment" });
  }
};
...
const updateEnroll = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Server Error - Unable to update enrollment" });
  }
};
